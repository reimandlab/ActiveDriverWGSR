...
id = "ATM"
```
**Note** that when splitting tasks using the `ADWGS_test` function, only the parameter `id` needs to modified for each element while `gr_element_coords`, `gr_sites` and `gr_maf` can be complete datasets. However, compute time and memory can be saved in large analyses by providing subsets of data for each test, for example those limited to specific chromosomes.
```{r result}
